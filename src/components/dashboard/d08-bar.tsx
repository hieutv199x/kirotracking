...
export function D08Bar({ data }: { data: OverviewPayload }) {
  ...
  const takeover = formatShare(data.takeover_rate, "stories with takeover", "—");
    ...
    <section className="reveal panel panel-pad flex flex-col gap-3" style={{ animationDelay: "120ms" }}>
      <div className="flex flex-wrap items-center justify-between gap-2">
        <SectionHead
          ...
          hint="AI paused; human decided; AI continued. Solid = spec lock. Muted = review."
        />
      ...
      <div className="grid grid-cols-3 gap-3 text-sm">
        <Split n={data.d08_spec_lock_sum} total={total} label="Spec lock" />
        <Split n={data.d08_review_sum} total={total} label="Review" />
        <Split n={data.d08_other_sum} total={total} label="Other" />
      </div>
